...
spx_private void ydb_storage_dio_do_delete_form_chunkfile(struct ev_loop *loop,ev_async *w,int revents){/*{{{*/
    ...
    dc->metadata =  ydb_storage_dio_make_metadata(dc->log,true,
            dc->opver + 1,dc->ver,
            dc->createtime,spx_now(),
            dc->totalsize,dc->realsize,dc->suffix,dc->hashcode,&err);
    if(NULL == dc->metadata){
        SpxLog2(dc->log,SpxLogError,err,\
    ...
    memcpy(mptr + offset,dc->metadata->buf,YDB_CHUNKFILE_MEMADATA_SIZE);
